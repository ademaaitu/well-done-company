...
class EarthquakeSafetyTestController extends Controller
{
    ...
    public function prototypeSubmit(Request $request): JsonResponse
    {
        ...
        // Чек-лист формируется только по реальным пробелам
        $personalizedChecklist = [];

        // Квиз: добавляем пункт только если ответили неправильно
        $quizChecklistMap = [
            'q1' => 'Отработайте действия в ТРЦ при тряске: отойти от стекла, присесть у несущей стены, закрыть голову руками.',
            'q2' => 'Повторите правило ночного землетрясения дома: укрыться у внутренней стены, не выбегать по лестнице до окончания толчков.',
            'q3' => 'Запомните правило в машине: остановиться вдали от мостов, зданий и ЛЭП — и оставаться внутри.',
            'q4' => 'Закрепите алгоритм при запахе газа: не включать электричество, перекрыть газ, выйти, вызвать службу.',
            'q5' => 'Запомните правило под завалами: стучать по трубам периодически, экономить силы и голос, не кричать постоянно.',
        ];
        ...
        foreach ($validated['quiz_answers'] as $entry) {
            ...
            if ($question && $question['correct_option'] !== $selectedOption && isset($quizChecklistMap[$questionId])) {
                $personalizedChecklist[] = $quizChecklistMap[$questionId];
            }
        }

        // Стресс-кейс: добавляем пункт только если выбрали неоптимальный ответ
        $stressChecklistMap = [
            'priority'       => ['atrium'    => null, 'lift_first' => 'Отработайте приоритизацию: управляемая эвакуация атриума стоит на первом месте, лифт — отдельная задача параллельно.', 'wait' => 'Отработайте приоритизацию: управляемая эвакуация атриума стоит на первом месте, лифт — отдельная задача параллельно.'],
            'team_split'     => ['balanced'  => null, 'all_atrium' => 'Закрепите схему распределения сотрудников: 3 — атриум, 2 — фуд-корт, 1 — периметр и газ.', 'all_lift' => 'Закрепите схему распределения сотрудников: 3 — атриум, 2 — фуд-корт, 1 — периметр и газ.'],
            'evacuation'     => ['controlled'=> null, 'full_panic' => 'Изучите протокол секторной эвакуации: объявлять по секторам с командами через рацию, а не общим сигналом.', 'no' => 'Изучите протокол секторной эвакуации: объявлять по секторам с командами через рацию, а не общим сигналом.'],
            'responsibility' => ['take'      => null, 'delegate' => 'Отработайте принятие решений под давлением: фиксируйте приказы и берите ответственность — промедление увеличивает риск.', 'delay' => 'Отработайте принятие решений под давлением: фиксируйте приказы и берите ответственность — промедление увеличивает риск.'],
        ];

        foreach ($validated['stress_answers'] as $key => $selected) {
            $item = $stressChecklistMap[$key][$selected] ?? null;
            if ($item !== null) {
                $personalizedChecklist[] = $item;
            }
        }

        // Сценарий: добавляем только при высоком риске
        if ($scenarioRisk === 'high') {
            $personalizedChecklist[] = 'Повторите сценарий у эскалатора: уход от толпы и укрытие вдали от стекла.';
        }

        // Если всё правильно — добавляем один общий пункт
        if (empty($personalizedChecklist)) {
            $personalizedChecklist[] = 'Отличный результат! Проведите короткий разбор сценария с командой для закрепления навыков.';
        }
